...
export class Calculator extends Operations {
    ...
    toggleSecondFunction() {
        ...
        const normal = document.querySelectorAll('[data-action="sin"], [data-action="cos"], [data-action="tan"]');
        const inverse = document.querySelectorAll('[data-action="sin^-1"], [data-action="cos^-1"], [data-action="tan^-1"]');
    ...
    plusMinus() {
        ...
        this.displayValue = this.displayValue.replace(/-?[\d.]+$/, this.currentValue);
        this.updateScreen();
    }
    // Advanced operations
    square() {
    ...
    evaluateExpression() {
        try {
            ...
            expr = expr.replace(/(^|[^a])sin\(([^)]+)\)/g, (match, p1, p2) => p1 +
                (this.isDegreeMode
                    ? `Math.sin(${this._degToRad(parseFloat(p2))})`
                    : `Math.sin(${p2})`));
            expr = expr.replace(/(^|[^a])cos\(([^)]+)\)/g, (match, p1, p2) => p1 +
                (this.isDegreeMode
                    ? `Math.cos(${this._degToRad(parseFloat(p2))})`
                    : `Math.cos(${p2})`));
            expr = expr.replace(/(^|[^a])tan\(([^)]+)\)/g, (match, p1, p2) => p1 +
                (this.isDegreeMode
